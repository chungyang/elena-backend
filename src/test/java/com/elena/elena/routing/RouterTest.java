...
public class RouterTest {
	...
	public void dijkstraTest() {
		List<AbstractElenaPath> shortestPaths = dijkstra_router.getRoute(graph.getNode("0").get(), graph.getNode("3").get(), graph);
	...
	@Test
	public void yenTest() {
		List<AbstractElenaPath> shortestPaths = yen_router.getRoute(graph.getNode("0").get(), graph.getNode("3").get(), graph);
	...
	public void astarTest() {
		List<AbstractElenaPath> shortestPaths = astar_router.getRoute(graph.getNode("0").get(), graph.getNode("3").get(), graph);
		Float expected = 3f;
		Float actual = shortestPaths.get(0).getPathWeights().get(WeightType.DISTANCE);
		assertEquals(expected, actual);
	}
	...
	public void yen_astar_router() {
		List<AbstractElenaPath> shortestPaths = yen_astar_router.getRoute(graph.getNode("0").get(), graph.getNode("3").get(), graph);
		Float expected = 5f;
		Float actual = shortestPaths.get(2).getPathWeights().get(WeightType.DISTANCE);
		assertEquals(expected, actual);
	}
